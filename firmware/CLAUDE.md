...
**Calibration mode:** holding SW1 during a cold power-up (wakeup cause is not `ext0`, since SW1 is always pressed on a normal wake) runs `runCalibration()` instead of the cycle above: release SW1, aim at white and press SW1, aim at black and press SW1, then sleep. The white step saves the raw reading as the firmware's own white point and also runs the module's `calibrateWhiteBalance()` + `saveToFlash()`. The black step uses `calibrateBlack()`. Each step aborts after `CAL_STEP_TIMEOUT_MS`. The DFPlayer is powered for the whole calibration and plays spoken prompts: `0011.mp3` (aim at white), `0012.mp3` (aim at black), `0013.mp3` (complete), in `TRACK_CAL_*`. Each step waits for its prompt to finish before listening for SW1. The same prompts also go to Serial. If the DFPlayer fails to start, calibration still runs, just without audio. Both points are stored in NVS (`Preferences`, namespace `iwand`, keys `white` and `black`) and reloaded by `loadCalibration()` on every wake. Tested on hardware: the calibration is still there after deep sleep and after a full power cut.
...
- `void playMp3Folder(int fileNumber)` plays `/mp3/NNNN.mp3` by file name, and `void volume(uint8_t volume)`. The sketch doesn't use `play(int)`, because that picks files by the order they were copied onto the card, not by name. All audio files go in the SD card's `mp3` folder.
- SD card layout (recorded and tested on hardware): `0001`–`0010` are the colour names, in `trackForColour()` order: Black, White, Gray, Red, Orange, Yellow, Green, Cyan, Blue, Purple. `0011`–`0013` are the calibration prompts (`TRACK_CAL_*`). The sketch once had a `play(track + 1)` workaround, which is gone now that `playMp3Folder()` picks files by name.
- Playback-finished detection is event-based, not a blocking call: poll `available()`, and when true check `readType() == DFPlayerPlayFinished`. Stale events stay buffered: a prompt's "finished" event arrives while the sketch is waiting for SW1, and some modules send it twice. `playTrack()` therefore clears `available()`/`readType()` before each play. Without that, the wait returned straight away and the final calibration prompt was cut off by `stopPlayer()`.
